...
extension StaffRegistrationFormViewModel {
    ...
    func validateEmail() -> Bool {
        ...
        guard email.count <= 254 else {
            fieldErrors[.email] = "Email cannot be more than 254 characters long."
            return false
        }
    ...
    func validateDateOfBirth() -> Bool {
        ...
        let today = Date()
        guard let age = calendar.dateComponents([.year], from: birthDate, to: today).year else {
        ...
        if age > maxAge {
            fieldErrors[.dateOfBirth] = "Maximum age for \(getAgeRoleDescription(role)) is \(maxAge) years"
            return false
        }
